...
import { Loader2, ChevronRight, Plus, Trash2, Wand2, AlertCircle } from "lucide-react";
    ...
    <div className="flex flex-col h-[calc(100vh-100px)] -m-6 bg-slate-50/50">
          ...
          {validationErrors.length > 0 && (
            <div className="p-4 bg-rose-50 border border-rose-200 rounded-lg shadow-sm animate-in fade-in slide-in-from-top-2">
              <div className="flex items-center gap-2 mb-2 font-semibold text-rose-800 text-sm">
                <AlertCircle className="h-4 w-4" /> Please fix the following before saving:
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => updateProject("billingStrategy", v)}
                      >
                        <SelectTrigger className="h-10 border-slate-200">
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateProject("totalBudget", parseFloat(e.target.value) * 100)
                          }
              ...
              <Card className="shadow-sm border-slate-200 overflow-hidden flex flex-col h-full max-h-[600px]">
                      ...
                      <div className="flex gap-4">
                        <div className="flex-1 space-y-3 min-w-0">
                          <Input
                            ...
                            className="h-8 w-full border-transparent hover:border-slate-200 focus:border-indigo-500 px-0 font-medium text-slate-900 placeholder:text-slate-400 truncate"
                          ...
                          <div className="relative">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                updateMilestone(idx, "amount", parseFloat(e.target.value) * 100)
                              }
                              className="h-8 pl-8 border-slate-100 bg-slate-50 text-right text-xs font-mono w-full"
                            />
                          </div>
                          <div>
                            <Input
                              ...
                              className="h-8 w-full text-xs border-slate-100 bg-slate-50"
                            />
                          </div>
                          <div className="w-full">
                            <Select
                              value={milestone.status}
                              onValueChange={(v) => updateMilestone(idx, "status", v)}
                            >
                              <SelectTrigger className="h-8 border-slate-200 bg-slate-50 text-xs w-full">
                                <SelectValue placeholder="Status" />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="PLANNED">Planned</SelectItem>
                                <SelectItem value="IN_PROGRESS">In Progress</SelectItem>
                                <SelectItem value="READY_FOR_INVOICE">Ready to Invoice</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                        </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-white shrink-0">
                  <div className="flex justify-between items-end mb-2">
                    <div className="text-xs font-medium text-slate-500">Budget Utilization</div>
                    <div
                      className={cn(
                    ...
                    >
                      {data.project.currency} {(totalMilestoneAmount / 100).toLocaleString()}
                ...
                <CardContent className="p-6 space-y-5">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(c) => updateProject("autoInvoiceEnabled", c)}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-slate-500">Auto-schedule email reminders.</p>
                    </div>
                    <Switch
                      checked={data.project.autoRemindersEnabled}
                      onCheckedChange={(c) => updateProject("autoRemindersEnabled", c)}
                    />
                  </div>
                </CardContent>
              </Card>
      ...
      <div className="shrink-0 p-4 border-t border-slate-200 bg-white flex flex-col sm:flex-row justify-between items-center gap-4 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        <div className="flex w-full sm:w-auto items-center gap-3">
          <Button variant="ghost" onClick={onCancel} disabled={isSaving} className="text-slate-500">
            Cancel
          </Button>
          <Button
            onClick={onConfirm}
