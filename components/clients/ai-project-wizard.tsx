...
export function AiProjectWizard({
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      setData({
        ...
        milestones:
          mainProject.milestones?.map((m: any) => ({
  ...
  const validateProjectData = () => {
    ...
    if (data.project.billingStrategy === "SINGLE_INVOICE" && data.milestones.length > 1) {
      errors.push(
        "Single Invoice strategy should typically have only one milestone/payment event."
      );
    }
  ...
  const proceedWithSaving = async (existingClientId?: string, overrideProjectName?: string) => {
    ...
    try {
      ...
      if (overrideProjectName) {
        ...
        projectToSave.name === data.project.name
      ) {
        // simple fallback
      ...
      if (!clientId) {
        ...
        const clientPayload: Record<string, string> = {
          name: data.client.name || "Unnamed Client",
        };
        if (data.client.email && data.client.email.includes("@")) {
          clientPayload.email = data.client.email;
        }
        if (data.client.company) clientPayload.company = data.client.company;
        if (data.client.phone) clientPayload.phone = data.client.phone;
        if (data.client.notes) clientPayload.notes = data.client.notes;
      ...
      toast.success(
        existingClientId
          ? "Project merged into existing client!"
    ...
    try {
      ...
      if (defaultClientId) {
        ...
        if (res.ok) {
          ...
          targetClient = client;
          existingProj = client.projects?.find(
            (p: any) => p.name.toLowerCase().trim() === data.project.name.toLowerCase().trim()
          );
        }
      } else {
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {duplicateProjectName ? (
                <>
                  A project named <strong>&quot;{data?.project.name}&quot;</strong> already exists
                  for client <strong>{duplicateClient?.name}</strong>.
                </>
              ) : (
                <>
                  The client <strong>{duplicateClient?.name}</strong> already exists in your
                  database.
                </>
              )}
            </DialogDescription>
            ...
            <div className="bg-slate-50 p-3 rounded-lg text-sm border border-slate-100 space-y-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-medium">
                  {duplicateProject.currency}{" "}
                  {(duplicateProject.totalBudget / 100).toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500">Status:</span>
                <Badge variant="outline" className="text-[10px] h-5">
                  {duplicateProject.status || "Active"}
                </Badge>
              </div>
              <div className="flex justify-between items-center">
            ...
            {duplicateProjectName ? (
              <div className="flex flex-col gap-2 w-full">
                ...
                >
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-[10px] text-slate-500 font-normal">
                      Update details, keep milestones.
                    </span>
                  </div>
                </Button>
                <Button
                ...
                <Button
                  ...
                  variant="destructive"
                  className="w-full justify-start bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200"
                >
                  <div className="flex flex-col items-start">
                    <span className="font-semibold">Delete Old & Replace</span>
                    <span className="text-[10px] text-rose-600/80 font-normal">
                      Danger: Removes old data permanently.
                    </span>
                  </div>
                </Button>
              </div>
              ...
              <div className="flex flex-col gap-2 w-full">
                <Button
                  onClick={() => proceedWithSaving(duplicateClient?.id)}
