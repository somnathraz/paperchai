      ...
      {!compact && (
        <div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm font-semibold">
            ...
            <button className="rounded-full bg-gradient-to-r from-primary via-emerald-500 to-primary px-5 py-2 text-white shadow-primary/30 hover:shadow-primary/50">
              Add invoice
            </button>
            <button className="rounded-full border border-slate-200 bg-white px-3 py-2 text-slate-700">
              ⋯
            </button>
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant="secondary"
                  className="rounded-full px-2 text-xs font-normal text-slate-500"
                >
              ...
              <div className="flex gap-2">
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 rounded-full text-xs text-slate-500 hover:text-primary"
            ...
            <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="mt-4">
                      <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
                        <span>Progress</span>
                        <span>
                          {Math.round(progress)}% ({completedMilestones}/{totalMilestones})
                        </span>
                      </div>
                      <Progress
                        value={progress}
                      ...
                      <div className="mt-4 flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-slate-500">
                            Due:{" "}
                            {nextMilestone.expectedDate
                              ? new Date(nextMilestone.expectedDate).toLocaleDateString()
                              : "TBD"}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs font-bold text-slate-900">
                            ₹{(nextMilestone.amount / 100).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    )}
              ...
              {client.projects && client.projects.length === 0 && (
                <div className="text-center py-8 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  ...
                  <Button variant="link" className="text-blue-600" onClick={onNewProject}>
              ...
              {timelineMock.map((item, idx) => (
                <div key={idx} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div
                      className={`flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 ${item.color}`}
                    >
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {client?.invoices?.map((inv: any) => (
                    <tr key={inv.id} className="hover:bg-slate-50">
                      ...
                      <td className="py-3">
                        <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold capitalize">
                          {inv.status}
                        </span>
                      </td>
                      <td className="py-3">
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-700">
              {client.documents && client.documents.length > 0 ? (
                client.documents.map((doc: any) => (
                  <div
                    key={doc.id}
                    className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer group"
                  >
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:border-primary hover:text-primary hover:bg-slate-50 transition-all"
              >
