...
const THEMES = [
  {
    ...
    name: "Modern",
    color: "bg-slate-900",
    description: "Clean rounded cards",
    premium: false,
  },
  {
    id: "minimal",
    ...
    description: "Focus on typography",
    premium: false,
  },
  {
    id: "classic",
    ...
    description: "Traditional letter",
    premium: false,
  },
  {
    id: "noir",
    name: "Noir",
    color: "bg-black border-slate-800",
...
export function EmailTemplateEditor() {
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      ...
      const reminderTemplate = data.templates?.find(
        (t: EmailTemplate) => t.slug === "invoice-reminder"
      );
    ...
    <div className="flex flex-col gap-8 pb-10">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Customize how PaperChai follows up with your clients.
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card text-card-foreground shadow-sm">
            <div className="border-b p-6 pb-4">
              ...
              <p className="mt-1.5 text-sm text-muted-foreground">
                Choose the visual style of your email layout. This is what your client sees.
              </p>
            </div>
            <div className="p-6 pt-4">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {THEMES.map((theme) => (
                  ...
                  >
                    <div
                      className={cn(
                        "h-12 w-full rounded-md shadow-sm transition-transform group-hover:scale-105",
                        theme.color
                      )}
                    ></div>
                    <div>
              ...
              <p className="mt-4 text-[11px] text-muted-foreground">
                Applies to all reminder emails for this workspace.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card text-card-foreground shadow-sm">
            <div className="flex items-center justify-between border-b p-6 pb-4">
              <div>
                ...
                <p className="mt-1.5 text-sm text-muted-foreground">
                  The actual message sent to clients.
                </p>
              </div>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 gap-2 text-primary border-primary/20 bg-primary/5 hover:bg-primary/10"
                  >
                    {aiLoading ? (
                ...
                <DropdownMenuContent align="end" className="w-[180px]">
                  <DropdownMenuItem onClick={() => handleAiRewrite("polite")}>
                    Make it more polite
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleAiRewrite("firm")}>
                    Make it firmer
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleAiRewrite("short")}>
                    Shorten message
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleAiRewrite("friendly")}>
                    Make it friendly
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <div className="border-t bg-muted/10 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  >
                    <code className="text-[10px] text-muted-foreground group-hover:text-primary/70">
                      {v.key}
                    </code>
                    <span>{v.label}</span>
                  </button>
          ...
          <div className="sticky bottom-4 z-10 flex items-center justify-between rounded-xl border border-border bg-white/90 p-4 shadow-lg backdrop-blur-md">
            <div className="flex items-center gap-2 px-2">
              <div
                className={cn("h-2 w-2 rounded-full", isDirty ? "bg-amber-500" : "bg-emerald-500")}
              ></div>
              <span className="text-xs font-medium text-muted-foreground">
...
function EmailPreview({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-2xl border border-border bg-slate-50 shadow-sm",
        className
      )}
    >
      ...
      <div className="flex flex-1 items-center justify-center p-8 min-h-[400px] overflow-hidden bg-slate-100/50">
        <div
          className={cn(
            ...
            device === "mobile"
              ? "w-[320px] h-[580px] rounded-[24px] border-[6px] border-slate-800"
              : "w-full h-full rounded-lg border border-border/20"
          )}
        >
          <iframe
            title="Email Preview"
