...
export function AIReviewPanel({
  ...
  const fixableIssues =
    reviewResult?.issues.filter((i) => i.autoFix && !appliedFixes.has(i.id)) || [];
        ...
        {isLoading ? (
          ...
          <div className="p-4 space-y-4">
            ...
            <div
              className={cn(
                ...
                riskConfig[reviewResult.riskLevel].bg === "bg-emerald-500"
                  ? "bg-emerald-50 border-emerald-200"
                  : riskConfig[reviewResult.riskLevel].bg === "bg-amber-500"
                    ? "bg-amber-50 border-amber-200"
                    : riskConfig[reviewResult.riskLevel].bg === "bg-orange-500"
            ...
            >
              ...
              <div className="flex items-end gap-2">
                <span
                  className={cn("text-4xl font-bold", riskConfig[reviewResult.riskLevel].color)}
                >
              ...
              <div className="mt-3 h-2 bg-slate-200 rounded-full overflow-hidden">
                <div
                  className={cn(
                    ...
                    riskConfig[reviewResult.riskLevel].bg
                  )}
                  ...
                  {reviewResult.issues.length === 0 ? (
                    ...
                    reviewResult.issues.map((issue) => {
                      ...
                      return (
                        <div
                          key={issue.id}
                          className={cn("px-4 py-3", config.bg, isApplied && "opacity-60")}
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span
                                  className={cn(
                                    "text-sm font-medium",
                                    isApplied ? "text-slate-500 line-through" : "text-slate-800"
                                  )}
                                >
                                  {issue.title}
                                </span>
                                <Badge
                                  variant="secondary"
                                  className={cn("text-[10px] h-4", config.badge)}
                                >
                                  {issue.severity}
                                </Badge>
                                {isApplied && (
                              ...
                              <p
                                className={cn(
                                  "text-xs mb-2",
                                  isApplied ? "text-slate-400" : "text-slate-600"
                                )}
                              >
                                {issue.description}
                              </p>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-amber-800">
                      Anomalies Detected ({reviewResult.anomalies.length})
                    </span>
                  </div>
                      ...
                      <div key={anomaly.id} className="mb-3 last:mb-0">
                        ...
                        {anomaly.metric && (
                          <div className="mt-1 text-xs text-amber-600">
                            Expected: {anomaly.metric.unit}{" "}
                            {anomaly.metric.expected.toLocaleString()} → Actual:{" "}
                            {anomaly.metric.unit} {anomaly.metric.actual.toLocaleString()}
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={cn(
                        ...
                        reviewResult.latePaymentProbability > 50
                          ? "text-red-600"
                          : reviewResult.latePaymentProbability > 25
                            ? "text-amber-600"
                            : "text-emerald-600"
                      )}
                    >
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4 text-center px-8">
            ...
            <p className="text-sm text-slate-500">
              Click &quot;AI Review&quot; to analyze your invoice
            </p>
          </div>
        )}
      </div>
