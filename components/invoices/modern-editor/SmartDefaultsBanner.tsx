...
export function SmartDefaultsBanner({
  ...
  return (
    <div className={cn("rounded-lg border p-3 overflow-hidden", config.color, className)}>
      {/* Header */}
...
export function RecommendationChip({
  ...
  return (
    <button
      ...
      title={source}
    >
      {applied ? <Check className="h-2.5 w-2.5" /> : <Sparkles className="h-2.5 w-2.5" />}
      {label}
    </button>
  );
}
...
export function DueDateRecommendation({
  ...
  return (
    ...
    >
      <Sparkles
        className={cn("h-4 w-4 shrink-0", applied ? "text-emerald-600" : "text-amber-600")}
      />
      <div className="flex-1 min-w-0">
        <p className={cn("text-xs font-medium", applied ? "text-emerald-800" : "text-amber-800")}>
      ...
      <Button
        ...
        size="sm"
        className={cn("h-6 text-xs shrink-0", applied && "text-emerald-600 pointer-events-none")}
        onClick={onApply}
        disabled={applied}
      >
        {applied ? (
