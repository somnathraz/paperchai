...
export function ClassicGray({
  ...
  const renderBlock = (id: string) => {
    switch (id) {
      case "header":
        ...
        return (
          <div
            className="flex items-start justify-between border-b px-12 pt-12 pb-8"
          ...
          >
              ...
              {data.logoUrl ? (
                ...
                <img
                  ...
                  style={{
                    ...
                    borderRadius:
                      data.logoSettings?.style === "square"
                        ? "0px"
                        : data.logoSettings?.style === "circle"
                          ? "9999px"
                          : "4px",
                  }}
                />
                ...
                <div
                  className="h-12 w-12 rounded mb-3"
                  style={{ backgroundColor: primary + "20" }}
                />
              )}
              <h1
                className="leading-tight"
                style={{ color: primary, ...getTypo("header", "18px", "600") }}
              >
                {data.businessName}
              </h1>
            </div>
            <div className="text-right">
              <h2
                className="text-[22px] font-semibold mb-2 tracking-tight uppercase"
                style={{ color: accent }}
              >
      ...
      case "bill_to":
        ...
        return (
          <div
            className="border-b px-12 py-6 flex justify-between items-start"
            style={{ borderColor: primary + "20" }}
          >
            <div>
              <h3
              ...
              <p
                ...
                style={{ color: primary, ...getTypo("client", "14px", "500") }}
              >
                {data.clientName}
              </p>
              {data.clientCompany && (
                <p className="text-[13px] leading-relaxed" style={{ color: primary + "AA" }}>
                  {data.clientCompany}
                </p>
              )}
              {data.clientAddress && (
                <p className="text-[13px] leading-relaxed mt-1" style={{ color: primary + "88" }}>
                  {data.clientAddress}
                </p>
              )}
              {data.clientEmail && (
              ...
              {data.clientPhone && (
                <p className="text-[13px] leading-relaxed" style={{ color: primary + "88" }}>
                  {data.clientPhone}
                </p>
              )}
            </div>
            {data.projectName && (
              <div className="text-right">
                <p
                  className="text-[13px] font-medium leading-relaxed uppercase tracking-wide mb-1"
                  style={{ color: accent }}
                >
                  Project
                </p>
                <p
                  ...
                  style={{ color: primary, ...getTypo("client", "14px", "600") }}
                >
                  {data.projectName}
                </p>
              </div>
        ...
        return (
          <div className="border-b px-12 py-6" style={{ borderColor: primary + "20" }}>
            <div className="text-right space-y-1">
              {data.subtotal && (
                <p className="text-[13px]" style={{ color: primary + "AA" }}>
              ...
              {data.discount && (
                <p className="text-[13px] text-[#c2410c]">Discount: {data.discount}</p>
              )}
              {data.fee && (
                <p className="text-[13px]" style={{ color: primary + "AA" }}>
                  Fees: {data.fee}
              ...
              <div className="mt-2 pt-2 border-t" style={{ borderColor: primary + "10" }}>
                <p className="" style={{ color: accent, ...getTypo("total", "18px", "700") }}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b" style={{ borderColor: primary + "30" }}>
                  <th
                    className="text-left py-3 text-[12px] font-semibold uppercase tracking-wide"
                    style={{ color: accent }}
                  >
                    Description
                  </th>
                  <th
                    className="text-center py-3 text-[12px] font-semibold uppercase tracking-wide w-16"
                    style={{ color: accent }}
                  >
                    Qty
                  </th>
                  ...
                  <th
                    className="text-right py-3 text-[12px] font-semibold uppercase tracking-wide w-24"
                    style={{ color: accent }}
                  >
                    Rate
                  </th>
                ...
                {data.items?.map((item: any, idx: number) => (
                  <tr key={idx} className="border-b" style={{ borderColor: primary + "15" }}>
                    <td className="py-4" style={{ color: primary }}>
                      <div className="leading-relaxed" style={getTypo("items", "14px", "500")}>
                        {item.name}
                      </div>
                      {item.description && (
                        <div className="text-[12px] mt-1" style={{ color: primary + "88" }}>
                          {item.description}
                        </div>
                      )}
                      {item.hsnCode && (
                        <div className="text-[11px] mt-1" style={{ color: primary + "66" }}>
                          HSN: {item.hsnCode}
                        </div>
                      )}
                    </td>
                    <td className="py-4 text-[14px] text-center" style={{ color: primary }}>
                      {item.qty || item.quantity || 1}
                    </td>
                    <td
                      className="py-4 text-[12px] text-center capitalize"
                      style={{ color: primary + "AA" }}
                    >
                      {item.unit || "nos"}
                    </td>
                    <td className="py-4 text-[14px] text-right" style={{ color: primary }}>
                      {item.rate ? `₹${item.rate.toLocaleString()}` : "-"}
                    </td>
                    <td
                      className="py-4 text-right"
                      style={{ color: primary, ...getTypo("items", "14px", "500") }}
                    >
                      {item.amount}
                    </td>
                  </tr>
      ...
      case "notes":
        ...
        return (
          <div className="px-12 py-6 border-t" style={{ borderColor: primary + "20" }}>
            ...
            <p
              className="leading-relaxed"
              style={{ color: primary, ...getTypo("footer", "14px", "400") }}
            >
              {data.notes}
            </p>
          </div>
      ...
      case "payment":
        ...
        return (
          <div
            className="border-t px-12 py-6 space-y-2"
            style={{ borderColor: primary + "20", backgroundColor: primary + "08" }}
          >
            <p
              className="text-center"
              style={{ color: primary + "AA", ...getTypo("footer", "13px", "400") }}
            >
            ...
            {data.paymentInstructions && (
              <p
                className="text-[12px] text-center whitespace-pre-line"
            ...
            {data.paymentLinkUrl && (
              <p className="text-[12px] text-center break-all" style={{ color: primary + "88" }}>
                Payment link: {data.paymentLinkUrl}
              </p>
            )}
            ...
            {data.signatureUrl && (
              <div className="flex justify-center pt-2">
                <div
                  className="relative"
                  style={{
                    height: data.signatureSettings?.height
                      ? `${data.signatureSettings.height}px`
                      : "48px",
                ...
                >
                  <Image
                    ...
                    style={{
                      borderRadius:
                        data.signatureSettings?.style === "square"
                          ? "0px"
                          : data.signatureSettings?.style === "rounded"
                            ? "4px"
                            : "0px",
                    }}
                  />
  ...
  const orderedSections =
    sections && sections.length > 0
      ? sections
      : getOrderedSections("classic-gray", undefined, [
          "header",
          "bill_to",
          "summary",
          "items",
      ...
      <div className="h-[140px] rounded-xl overflow-hidden border border-white/30 bg-gradient-to-b from-slate-50 to-white p-4 shadow-inner">
        <div className="h-full rounded-xl border border-white/40 bg-white/80 shadow-inner p-4">
          <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-slate-500">
            <span>Invoice</span>
            <span className="rounded-full bg-emerald-100 text-emerald-700 px-2 py-1 text-[10px] font-semibold">
              Free
            </span>
          </div>
  ...
  if (modalPreview) {
    ...
    return (
      <div
        className="h-full w-full text-[#111111] flex flex-col"
        style={{ fontFamily, ...paperStyle }}
      >
                  ...
                  <table className="w-full text-[13px]">
                    <tbody>
                      {section.items.map((row, idx) => (
                        <tr key={idx} className="border-b" style={{ borderColor: primary + "15" }}>
                          <td className="py-2 pr-4 font-semibold" style={{ color: primary + "AA" }}>
                            {row.label}
                          </td>
                          <td className="py-2 text-right" style={{ color: primary }}>
                            {row.value}
                          </td>
                        </tr>
                      ))}
        ...
        {orderedSections.map((section) => {
          ...
          if (section.custom) {
            return (
              ...
              >
                <h3 className="text-sm font-semibold mb-2" style={{ color: primary + "AA" }}>
                  {section.title}
                </h3>
                {section.items && section.items.length > 0 ? (
                  ...
                  <p className="text-sm whitespace-pre-line" style={{ color: primary }}>
                    {section.content}
                  </p>
                )}
              </div>
