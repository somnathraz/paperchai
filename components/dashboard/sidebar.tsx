...
export function SidebarContent({ className, onNavigate }: { className?: string, onNavigate?: () => void }) {
  ...
  return (
    <div className={`relative flex h-full w-full max-w-full flex-col overflow-x-hidden bg-white/80/70 py-6 backdrop-blur-xl ${className}`}>
      <div className="flex items-center gap-2 px-6">
        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 via-primary/70 to-emerald-500/60 text-primary-foreground shadow-inner shadow-primary/30">
        ...
        {links.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onNavigate}
              className={`group flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold transition ${active
