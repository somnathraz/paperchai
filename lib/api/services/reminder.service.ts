...
export const reminderService = {
  ...
  sendReminder: async (invoiceId: string, channel: string) => {
    return apiClient.post(API_ENDPOINTS.REMINDERS.SEND, { invoiceId, channel });
  },

  /**
   * Retry/skip/dismiss a reminder step
   */
  applyAction: async (action: "retry" | "skip" | "dismiss", stepId: string) => {
    return apiClient.post(API_ENDPOINTS.REMINDERS.ACTION, { action, stepId });
  },
