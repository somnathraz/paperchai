...
export async function requireWorkspaceAccess(
  ...
  WorkspaceContext["workspace"] & { membership: WorkspaceContext["membership"]; isOwner: boolean }
> {
...
export function requireRole(
  ...
  if (!requiredRoles.includes(userRole)) {
    throw new AuthError(`Insufficient permissions. Required: ${requiredRoles.join(" or ")}`, 403);
  }
}
...
export async function getWorkspaceContext(
  ...
  if (requiredPermission) {
    const allowedRoles = securityConfig.workspaceRoles[
      requiredPermission
    ] as readonly WorkspaceRole[];
