...
export async function checkAiBudget(
  ...
  return {
    ...
    error:
      remaining <= 0
        ? `Daily AI limit reached (${dailyLimit} calls). Resets at midnight.`
        : undefined,
  };
}
 ...
 */
export function getCacheKey(workspaceId: string, contentHash: string, action: string): string {
  return crypto
    .createHash("sha256")
 ...
 */
export function cacheResult(cacheKey: string, result: unknown, action: string): void {
