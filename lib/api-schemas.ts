...
const phoneSchema = z
  .string()
  .regex(/^[\d\s+\-().ext]+$/, "Invalid phone format")
  .max(30)
  .optional();
...
export const invoiceSaveSchema = z.object({
  ...
  // Items
  items: z.array(invoiceItemSchema).max(securityConfig.validation.maxInvoiceItems).optional(),
...
export const reminderScheduleCreateSchema = z.object({
  ...
  steps: z.array(reminderStepSchema).max(securityConfig.validation.maxReminderSteps),
});
