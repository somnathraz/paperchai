...
export const fileUploadSchema = z.object({
  filename: z.string().max(255).refine(isFilenameSafe, "Invalid filename"),
  mimeType: z
    .string()
    .refine((type) => ALL_ALLOWED_TYPES.includes(type as any), "File type not allowed"),
  size: z.number().max(FILE_SIZE_LIMITS.document, "File too large"),
});
...
export const imageUploadSchema = z.object({
  ...
  size: z.number().max(FILE_SIZE_LIMITS.image, "Image too large (max 5MB)"),
});
...
export const logoUploadSchema = z.object({
  ...
  size: z.number().max(FILE_SIZE_LIMITS.logo, "Logo too large (max 2MB)"),
});
