...
export function ProjectDetailWidget({ projectId }: ProjectDetailWidgetProps) {
  ...
  const { selectedProject, isLoading, error, fetchProject, updateProject, clearSelected } =
    useProjects();
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8 bg-slate-50 min-h-screen">
      <div className="mb-6">
        <Link
          href={`/clients?clientId=${project.clientId}`}
          className="text-sm text-slate-500 hover:text-primary flex items-center mb-4"
        >
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2 text-slate-600">
              <span className="flex items-center">
                <FileText className="h-4 w-4 mr-1" /> {project.client?.name}
              </span>
              {project.totalBudget && (
            ...
            <div>
              ...
              <p className="font-semibold">
                {project.milestones?.filter((m: any) =>
                  ["INVOICED", "PAID", "READY_FOR_INVOICE"].includes(m.status)
                ).length || 0}{" "}
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      `/invoices/new?projectId=${project.id}&clientId=${project.clientId}`
                    )
                  }
                >
                ...
                {project.invoices?.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-slate-400 mt-1">
                      Create one manually or enable auto-invoicing for milestones.
                    </p>
                  </div>
                ) : (
                  project.invoices?.map((inv: any) => (
                    <div
                      key={inv.id}
                      className="flex justify-between items-center p-3 border rounded-lg hover:bg-slate-50 cursor-pointer transition-colors group"
                      onClick={() => router.push(`/invoices/new?id=${inv.id}`)}
                    >
                      <div className="flex items-center gap-4">
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-medium text-blue-600 group-hover:text-blue-800">
                              {inv.number}
                            </p>
                      ...
                      <div className="text-right flex items-center gap-4">
                        <p className="font-semibold">
                          {project.currency} {Number(inv.total).toFixed(2)}
                        </p>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {inv.status === "sent" && (
                              <DropdownMenuItem
                                ...
                                disabled={isUpdatingInvoice === inv.id}
                            ...
                            {inv.status === "paid" && (
                              <DropdownMenuItem
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleUpdateInvoiceStatus(inv.id, "sent");
                                }}
                                disabled={isUpdatingInvoice === inv.id}
                              >
