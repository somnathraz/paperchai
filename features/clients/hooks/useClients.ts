...
export const useClients = () => {
  ...
  const filteredClients = useMemo(() => {
    ...
    if (filters.reliability !== "all") {
      result = result.filter((c) => {
        ...
        if (filters.reliability === "sometimes")
          return c.reliabilityScore >= 60 && c.reliabilityScore < 80;
  ...
  return {
    ...
    setSort: (field: any, direction: "asc" | "desc") =>
      dispatch(setFilters({ sortField: field, sortDirection: direction })),
    toggleRow: (id: string) => dispatch(toggleRowExpansion(id)),
    selectClient: (client: any | null) => dispatch(setActiveClient(client)),
    storeClientProjects: (clientId: string, projects: any[]) =>
      dispatch(setClientProjects({ clientId, projects })),
  };
};
