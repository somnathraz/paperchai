...
export function ClientListWidget({
  ...
  // Local state for loading individual row projects
  const [loadingProjects, setLoadingProjects] = useState<Set<string>>(new Set());

  // Handle row expansion
  const handleToggleRow = async (clientId: string) => {
    toggleRow(clientId);
    // If expanding and no projects loaded, fetch them
    if (!expandedRows.includes(clientId) && !clientProjects[clientId]) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.client?.projects) {
            storeClientProjects(clientId, data.client.projects);
          }
        }
      } catch (err) {
        console.error(err);
                ...
                <TableRow className="bg-slate-50/50">
                  ...
                  <TableHead className="text-right">
                    <button
                      ...
                      className="flex items-center gap-2 font-semibold hover:text-primary transition-colors justify-end w-full"
                    >
                        ...
                        <TableCell>
                          <ChevronRight
                            className={cn(
                              ...
                              expandedRows.includes(client.id) ? "rotate-90" : ""
                            )}
                          />
                        </TableCell>
                        <TableCell>
                                ...
                                <div className="rounded-md border border-slate-200 bg-white shadow-sm overflow-hidden">
                                  <Table>
                                    <TableHeader className="bg-slate-50">
                                      <TableRow>
                                        <TableHead className="w-[250px] pl-4">
                                          Project Name
                                        </TableHead>
                                    ...
                                    <TableBody>
                                      {clientProjects[client.id].map((project: any) => (
                                        <TableRow
                                          key={project.id}
                                          className="hover:bg-slate-50 group/project"
                                        >
                                          ...
                                          <TableCell className="text-right font-mono text-slate-600">
                                            {project.totalBudget
                                              ? `₹${(project.totalBudget / 100).toLocaleString()}`
                                              : "—"}
                                          </TableCell>
