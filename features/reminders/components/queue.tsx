...
export function Queue() {
  const { filteredQueue, upcoming, filters, isLoading, sendReminder, refreshData } = useReminders();
  ...
  const handleResend = async (invoiceId: string, channel: string) => {
    ...
    try {
      await sendReminder(invoiceId, channel);
  ...
  const handleStepAction = async (stepId: string, action: "retry" | "skip" | "dismiss") => {
    if (!stepId) {
      toast.error("Reminder step id is missing");
      return;
    }
    ...
    try {
      ...
      toast.success(action === "retry" ? "Reminder queued for retry" : "Reminder skipped");
      refreshData();
    } catch (error) {
    ...
    <>
      <div className="rounded-3xl border border-border bg-card/50 backdrop-blur-sm overflow-hidden min-h-[400px]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {filteredQueue.length} reminders scheduled
              </p>
            </div>
          </div>
          <button
        ...
        <div className="divide-y divide-border/50">
          {filteredQueue.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground text-sm">
              {filters.statuses.length || filters.clients.length || filters.channels.length
            ...
            filteredQueue.map((item: any) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-[140px]">
                  <div
                    className={`flex items-center justify-center h-6 w-6 rounded-full ${item.channel === "email" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"}`}
                  >
                ...
                <div className="min-w-[110px]">
                  ...
                  {item.status === "skipped" && (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-500/10 px-2.5 py-0.5 text-xs font-medium text-gray-600">
                ...
                <div className="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                  <Sheet>
                    ...
                    <SheetContent className="w-[400px] sm:w-[540px]">
                      <TimelineDrawerContent invoiceId={item.invoiceId} clientName={item.client} />
                    </SheetContent>
                  </Sheet>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        <Send
                          className={`mr-2 h-4 w-4 ${sendingId === item.invoiceId ? "animate-spin" : ""}`}
                        />
                      ...
                      <DropdownMenuItem
                        ...
                        disabled={actioningId === item.stepId}
                        onClick={() => handleStepAction(item.stepId, "skip")}
                      >
        ...
        <div className="bg-muted/20 px-6 py-3 border-t border-border/50">
          <p className="text-xs text-muted-foreground">
            Use <span className="font-medium">View</span> on any row to inspect reminder timeline
            and send history.
          </p>
        </div>
      </div>

      <CalendarModal open={calendarOpen} onOpenChange={setCalendarOpen} invoices={upcoming} />
    </>
  );
}
