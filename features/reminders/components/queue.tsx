...
export function Queue() {
    const { queue, upcoming, isLoading, sendReminder } = useReminders();
    ...
    const handleResend = async (invoiceId: string, channel: string) => {
        ...
        try {
            await sendReminder(invoiceId, channel); // Thunk handles success/error logic internally if we wanted, but here we just dispatch
