        ...
        <div className="rounded-2xl border border-border bg-card/50 p-4 backdrop-blur-sm">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold">
              {typeof health?.openRate === "number" ? `${health.openRate}%` : "--"}
            </span>
            <span className="text-xs font-medium text-muted-foreground flex items-center">
              <MousePointerClick className="h-3 w-3 mr-0.5" />
              {typeof health?.openRate === "number" ? "Tracked" : "Not tracked"}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3 relative z-10">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              Delivery and failure metrics below are computed from live reminder runs (last 30
              days).
            </p>
          </div>
        </div>
