            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
              {selectedDayInvoices.length > 0 ? (
                selectedDayInvoices.map((inv, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 rounded-xl bg-muted/20 border border-border group"
                  >
                    <div className="space-y-1">
                      <p className="text-sm font-medium">{inv.client}</p>
                      <p className="text-xs text-muted-foreground">
                        Invoice #{inv.id} • ${(inv.amount || 0).toLocaleString()}
                      </p>
                    </div>
                    ...
                    <Sheet>
                      ...
                      <SheetContent className="w-[400px] sm:w-[540px]">
                        <TimelineDrawerContent invoiceId={inv.invoiceId} clientName={inv.client} />
                      </SheetContent>
                    </Sheet>
                  </div>
                ))
