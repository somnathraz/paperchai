...
export function UpcomingReminders() {
  const { upcoming, isLoading } = useReminders();
  ...
  useEffect(() => {
    if (!isLoading) {
      // Generate next 7 days based on upcoming data from Redux
      const next7Days = Array.from({ length: 7 }).map((_, i) => {
        const d = addDays(new Date(), i);
        const dayInvoices: UpcomingInvoice[] = upcoming
          .filter((u: any) => isSameDay(parseISO(u.date), d))
          .flatMap((u: any) => u.invoices);
          ...
          <div className="grid grid-cols-7 gap-2">
            {days.map((item, index) => (
              ...
              >
                ...
                <span
                  className={`text-sm font-semibold mt-1 ${item.hasReminders ? "text-foreground" : "text-muted-foreground/50"}`}
                >
                  {item.date.split(" ")[0]}
                </span>
                {item.hasReminders ? (
