...
export function StatsCards() {
      ...
      <section className="w-full max-w-full overflow-hidden">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-32 rounded-2xl bg-white/50 border border-white/60 animate-pulse"
            />
          ))}
        </div>
      </section>
  ...
  const cards = [
    ...
    },
    {
      ...
      delta: stats.averagePaymentTime
        ? stats.averagePaymentTime < 10
          ? "Fast"
          : stats.averagePaymentTime < 20
            ? "Average"
            : "Slow"
        : "No data",
      icon: Timer,
