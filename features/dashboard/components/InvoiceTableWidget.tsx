...
function statusClass(status: string) {
  ...
  if (status === "Partial Paid") return "bg-amber-50 text-amber-700";
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1">
                    <span
                      className={`inline-flex w-fit rounded-full px-2 py-1 text-xs font-semibold ${statusClass(inv.displayStatus)}`}
                    >
                      {inv.displayStatus}
                    </span>
                    {inv.displayStatus === "Partial Paid" ? (
                      <span className="text-[11px] text-amber-700">
                        ...
                        }).format(Number(inv.amountPaid || 0))}{" "}
                        · Due{" "}
                        {new Intl.NumberFormat("en-IN", {
                          style: "currency",
                          currency: "INR",
                          maximumFractionDigits: 0,
                        }).format(Number(inv.balanceDue || 0))}
                      </span>
                    ) : null}
                  </div>
                </td>
