    ...
    if (documents.length === 0) {
      ...
      if (pendingDocs.length === 0) {
        return NextResponse.json({
          ...
          warnings: [
            "No documents found for this project. Upload a contract or agreement to extract billable items.",
          ],
        });
      }
      ...
      return NextResponse.json({
        ...
        totalDocumentsAnalyzed: 0,
        warnings: [
          `${pendingDocs.length} document(s) are still being processed. Please try again in a few moments.`,
        ],
      });
    }
...
`;
    ...
    try {
      ...
      return NextResponse.json({
        ...
        warnings: [
          "AI extraction returned invalid format. Please try again or add items manually.",
        ],
      });
    }
