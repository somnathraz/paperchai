...
import { ProjectType, BillingStrategy, MilestoneBillingTrigger, MilestoneStatus } from "@prisma/client";
...
export async function POST(
    ...
    try {
        ...
        if (project.autoInvoiceEnabled && project.milestones && project.milestones.length > 0) {
            let eligibleMilestones = project.milestones.filter(m =>
                m.billingTrigger === "ON_CREATION" && m.autoInvoiceEnabled
            );
...
async function generateInvoiceForMilestone(workspaceId: string, clientId: string, projectId: string, milestone: any) {
