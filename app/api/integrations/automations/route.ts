...
export async function GET() {
  try {
    ...
    // Use ensureActiveWorkspace to auto-create if needed
    const workspace = await ensureActiveWorkspace(session.user.id, session.user.name);
    if (!workspace) {
      // Return empty data if workspace not ready yet
      return NextResponse.json({
        success: true,
        automations: { running: [], completed: [], failed: [], total: 0 },
      });
    }
    ...
    const automations = [
      ...notionImports.map((imp) => ({
        ...
        status:
          imp.status === "COMPLETED" ? "completed" : imp.status === "FAILED" ? "failed" : "running",
      ...
      ...slackImports.map((imp) => ({
        ...
        details:
          imp.errorMessage || imp.aiSummary || `Processing Slack ${imp.importType.toLowerCase()}`,
      })),
