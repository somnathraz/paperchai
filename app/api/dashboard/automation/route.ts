  ...
  try {
    ...
    if (!workspace) {
      ...
      return NextResponse.json({
        todaySummary: { emailsSent: 0, remindersFired: 0, invoicesCreated: 0, invoicesPaid: 0 },
        pipeline: {
          draft: 0,
          scheduled: 0,
          sent: 0,
        ...
        recentlyCompleted: [],
        errors: [],
        revenue: { totalCollected: 0, timeSavedHours: 0 },
        activity: [],
      });
    }
    ...
    const getCount = (status: string) =>
      invoiceCounts.find((c) => c.status === status)?._count.id || 0;
