...
export async function GET() {
  try {
    ...
    if (!workspace) {
      // Return default empty stats if workspace not ready
      return NextResponse.json({
        ...
        reliabilitySparkline: [0, 0, 0, 0, 0],
        thisMonthPaid: 0,
        lastMonthPaid: 0,
      });
    }
    ...
    const payoutSparkline = paidWithDates.slice(-5).map((inv) => {
      const diff = (inv.updatedAt.getTime() - inv.issueDate!.getTime()) / (1000 * 60 * 60 * 24);
      return Math.max(0, Math.min(20, diff));
    });

    const reliabilitySparkline = clients.slice(-5).map((c) => c.reliabilityScore ?? 0);
    ...
    const thisMonthPaid = paidInvoices
      .filter((i) => i.updatedAt >= thisMonth)
      .reduce((sum, i) => sum + Number(i.total || 0), 0);
    const lastMonthPaid = paidInvoices
      .filter((i) => i.updatedAt >= lastMonth && i.updatedAt < thisMonth)
      .reduce((sum, i) => sum + Number(i.total || 0), 0);
    ...
    const stats = {
      ...
      reliabilitySparkline:
        reliabilitySparkline.length > 0 ? reliabilitySparkline : [0, 0, 0, 0, 0],
